...
impl From<U256> for f64 {
	fn from(value: U256) -> f64 {
		let (value, factor) = match value {
			U256([_, _, 0, 0]) => (value, 1.0),
			U256([_, _, _, 0]) => (value >> 64, 2.0f64.powi(64)),
			U256([_, _, _, _]) => (value >> 128, 2.0f64.powi(128)),
		};
		(value.low_u128() as f64) * factor
	}
}
