...
fn trie_insertions_32_mir_1k(b: &mut Criterion) {
	...
	let d = st.make();
	b.bench_function("trie_insertions_32_mir_1k", |b| b.iter(&mut ||{
...
fn trie_insertions_32_ran_1k(b: &mut Criterion) {
	...
	b.bench_function("trie_insertions_32_ran_1k", |b| b.iter(&mut ||{
		let mut memdb = MemoryDB::<KeccakHasher, DBValue>::new_codec();
		let mut root = H256::default();
		let mut t = TrieDBMut::new(&mut memdb, &mut root);
		for i in d.iter() {
...
fn trie_insertions_six_high(b: &mut Criterion) {
	...
	b.bench_function("trie_insertions_six_high", |b| b.iter(&mut ||{
		let mut memdb = MemoryDB::<KeccakHasher, DBValue>::new_codec();
		let mut root = H256::default();
		let mut t = TrieDBMut::new(&mut memdb, &mut root);
		for i in d.iter() {
...
fn trie_insertions_six_mid(b: &mut Criterion) {
	...
	b.bench_function("trie_insertions_six_mid", |b| b.iter(&mut ||{
		let mut memdb = MemoryDB::<KeccakHasher, DBValue>::new_codec();
		let mut root = H256::default();
		let mut t = TrieDBMut::new(&mut memdb, &mut root);
		for i in d.iter() {
...
fn trie_insertions_random_mid(b: &mut Criterion) {
	...
	b.bench_function("trie_insertions_random_mid", |b| b.iter(&mut ||{
		let mut memdb = MemoryDB::<KeccakHasher, DBValue>::new_codec();
		let mut root = H256::default();
		let mut t = TrieDBMut::new(&mut memdb, &mut root);
		for i in d.iter() {
...
fn trie_insertions_six_low(b: &mut Criterion) {
	...
	b.bench_function("trie_insertions_six_low", |b| b.iter(&mut ||{
		let mut memdb = MemoryDB::<KeccakHasher, DBValue>::new_codec();
		let mut root = H256::default();
		let mut t = TrieDBMut::new(&mut memdb, &mut root);
		for i in d.iter() {
